		      ...
		      const struct nlattr *attr)
{
	...
	if (swkey->eth.type == htons(ETH_P_VIRO)) {
		...
		if (!(attrs & (1 << OVS_KEY_ATTR_VIRO)))
			return -EINVAL;
		attrs &= ~(1 << OVS_KEY_ATTR_VIRO);

		key_len = SW_FLOW_KEY_OFFSET(viro);
		viro_key = nla_data(a[OVS_KEY_ATTR_VIRO]);

		swkey->viro.fd_sw = viro_key->sw_fd;
		swkey->viro.fd_host = viro_key->host_fd;

		addr = (struct viro_addr *) swkey->eth.dst;
		addr->sw = viro_key->sw_dst;
		addr->host = viro_key->host_dst;

		addr = (struct viro_addr *) swkey->eth.src;
		addr->sw = viro_key->sw_src;
		addr->host = viro_key->host_src;
...
int ovs_flow_to_nlattrs(const struct sw_flow_key *swkey, struct sk_buff *skb)
{
	...
	if (swkey->eth.type == htons(ETH_P_VIRO)) {
		...
		addr = (struct viro_addr *) swkey->eth.src;
		viro_key->sw_src = addr->sw;
		viro_key->host_src = addr->host;;

		addr = (struct viro_addr *) swkey->eth.dst;
		viro_key->sw_dst = addr->sw;
		viro_key->host_dst = addr->host;

		viro_key->sw_fd = swkey->viro.fd_sw;
		viro_key->host_fd = swkey->viro.fd_host;
