...
static enum ofperr
decode_viro_ast_action(const union ofp_action *a, enum ofputil_action_code *code)
{
    const struct viro_action_header *vah = (const struct viro_action_header *) a;
    uint16_t len = ntohs(a->header.len);

    if (len < sizeof(struct viro_action_header)) {
        return OFPERR_OFPBAC_BAD_LEN;
    } else if (a->vendor.vendor != CONSTANT_HTONL(VIRO_VENDOR_ID)) {
        return OFPERR_OFPBAC_BAD_VENDOR;
    }

    switch (vah->subtype) {
#define VIRO_AST_ACTION(ENUM, STRUCT, EXTENSIBLE, NAME)    \
        case CONSTANT_HTONS(ENUM):                      \
            if (EXTENSIBLE                              \
                ? len >= sizeof(struct STRUCT)          \
                : len == sizeof(struct STRUCT)) {       \
                         ...
                         enum ofputil_action_code *code)
{
    ...
    case CONSTANT_HTONS(OFPAT10_VENDOR):
        err = decode_nxast_action(a, code);
    	if (err == OFPERR_OFPBAC_BAD_VENDOR)
    		return decode_viro_ast_action(a, code);
    	return err;
...
#define VIRO_AST_ACTION(ENUM, STRUCT, EXTENSIBLE, NAME) case OFPUTIL_##ENUM:
#include "ofp-util.def"
        NOT_REACHED();
...
static enum ofperr
ofpact_from_viro_ast(const union ofp_action *a, enum ofputil_action_code code,
                  struct ofpbuf *out)
{
    const struct viro_action_push_fd *viro_push_fd;
    const struct viro_action_vid_sw *viro_sw;
    const struct viro_action_vid_host *viro_host;

    enum ofperr error = 0;

    struct ofpact_viro_push_fd *ofp_push_fd;
...
#include "ofp-util.def"
    ...
    case OFPUTIL_VIRO_AST_PUSH_FD:
    	viro_push_fd = (struct viro_action_push_fd*) a;
    	ofp_push_fd = ofpact_put_VIRO_PUSH_FD(out);
    	ofp_push_fd->fd_sw = viro_push_fd->fd_sw;
    	ofp_push_fd->fd_host = viro_push_fd->fd_host;
        break;

    case OFPUTIL_VIRO_AST_POP_FD:
    	ofpact_put_VIRO_POP_FD(out);
    	break;

    case OFPUTIL_VIRO_AST_SET_VID_SW_FD:
    	viro_sw = (struct viro_action_vid_sw*) a;
    	ofpact_put_VIRO_SET_VID_SW_FD(out)->sw = viro_sw->sw;
    	break;

    case OFPUTIL_VIRO_AST_SET_VID_SW_SRC:
    	viro_sw = (struct viro_action_vid_sw*) a;
    	ofpact_put_VIRO_SET_VID_SW_SRC(out)->sw = viro_sw->sw;
    	break;

    case OFPUTIL_VIRO_AST_SET_VID_SW_DST:
    	viro_sw = (struct viro_action_vid_sw*) a;
    	ofpact_put_VIRO_SET_VID_SW_DST(out)->sw = viro_sw->sw;
    	break;

    case OFPUTIL_VIRO_AST_SET_VID_HOST_FD:
    	viro_host = (struct viro_action_vid_host*) a;
    	ofpact_put_VIRO_SET_VID_HOST_FD(out)->host = viro_host->host;
    	break;

    case OFPUTIL_VIRO_AST_SET_VID_HOST_SRC:
    	viro_host = (struct viro_action_vid_host*) a;
    	ofpact_put_VIRO_SET_VID_HOST_SRC(out)->host = viro_host->host;
    	break;

    case OFPUTIL_VIRO_AST_SET_VID_HOST_DST:
    	viro_host = (struct viro_action_vid_host*) a;
    	ofpact_put_VIRO_SET_VID_HOST_DST(out)->host = viro_host->host;
    	break;
    }
...
#define VIRO_AST_ACTION(ENUM, STRUCT, EXTENSIBLE, NAME) case OFPUTIL_##ENUM:
#include "ofp-util.def"
	return ofpact_from_viro_ast(a, code, out);
    }
...

static void
ofpact_to_viro_ast(const struct ofpact *a, struct ofpbuf *out)
{
	struct ofpact_viro_push_fd *ofpact_push_fd;
	struct ofpact_viro_set_vid_sw *ofpact_vid_sw;
	struct ofpact_viro_set_vid_host *ofpact_vid_host;

	struct viro_action_push_fd *push_fd;

    switch (a->type) {
    case OFPACT_VIRO_PUSH_FD:
    	ofpact_push_fd = ofpact_get_VIRO_PUSH_FD(a);
    	push_fd = ofputil_put_VIRO_AST_PUSH_FD(out);
    	push_fd->fd_sw = ofpact_push_fd->fd_sw;
    	push_fd->fd_host = ofpact_push_fd->fd_host;
        break;

    case OFPACT_VIRO_POP_FD:
    	ofputil_put_VIRO_AST_POP_FD(out);
    	break;

    case OFPACT_VIRO_SET_VID_SW_FD:
    	ofpact_vid_sw = ofpact_get_VIRO_SET_VID_SW_FD(a);
    	ofputil_put_VIRO_AST_SET_VID_SW_FD(out)->sw = ofpact_vid_sw->sw;
    	break;

    case OFPACT_VIRO_SET_VID_SW_SRC:
    	ofpact_vid_sw = ofpact_get_VIRO_SET_VID_SW_SRC(a);
    	ofputil_put_VIRO_AST_SET_VID_SW_SRC(out)->sw = ofpact_vid_sw->sw;
    	break;

    case OFPACT_VIRO_SET_VID_SW_DST:
    	ofpact_vid_sw = ofpact_get_VIRO_SET_VID_SW_DST(a);
    	ofputil_put_VIRO_AST_SET_VID_SW_DST(out)->sw = ofpact_vid_sw->sw;
    	break;

    case OFPACT_VIRO_SET_VID_HOST_FD:
    	ofpact_vid_host = ofpact_get_VIRO_SET_VID_HOST_FD(a);
    	ofputil_put_VIRO_AST_SET_VID_HOST_FD(out)->host = ofpact_vid_host->host;
    	break;

    case OFPACT_VIRO_SET_VID_HOST_SRC:
    	ofpact_vid_host = ofpact_get_VIRO_SET_VID_HOST_SRC(a);
    	ofputil_put_VIRO_AST_SET_VID_HOST_SRC(out)->host = ofpact_vid_host->host;
    	break;

    case OFPACT_VIRO_SET_VID_HOST_DST:
    	ofpact_vid_host = ofpact_get_VIRO_SET_VID_HOST_DST(a);
    	ofputil_put_VIRO_AST_SET_VID_HOST_DST(out)->host = ofpact_vid_host->host;
    	break;
...
static void
print_viro_action(const struct ofpact *a, struct ds *s)
{
    const struct ofpact_viro_push_fd *viro_push_fd;
    const struct ofpact_viro_set_vid_sw *viro_sw;
    const struct ofpact_viro_set_vid_host *viro_host;

    switch (a->type) {
    case OFPACT_VIRO_PUSH_FD:
    	viro_push_fd = ofpact_get_VIRO_PUSH_FD(a);
    	ds_put_format(s, "viro_push_fd(sw=%#"PRIx32",host=%#"PRIx16")",ntohl(viro_push_fd->fd_sw), ntohs(viro_push_fd->fd_host));
    	break;
    ...
    case OFPACT_VIRO_SET_VID_SW_FD:
    	viro_sw = ofpact_get_VIRO_SET_VID_SW_FD(a);
    	ds_put_format(s, "viro_set_vid_sw_fd:%#"PRIx32, ntohl(viro_sw->sw));
    	break;

    case OFPACT_VIRO_SET_VID_SW_SRC:
    	viro_sw = ofpact_get_VIRO_SET_VID_SW_SRC(a);
    	ds_put_format(s, "viro_set_vid_sw_src:%#"PRIx32, ntohl(viro_sw->sw));
    	break;

    case OFPACT_VIRO_SET_VID_SW_DST:
    	viro_sw = ofpact_get_VIRO_SET_VID_SW_DST(a);
    	ds_put_format(s, "viro_set_vid_sw_dst:%#"PRIx32, ntohl(viro_sw->sw));
    	break;

    case OFPACT_VIRO_SET_VID_HOST_FD:
    	viro_host = ofpact_get_VIRO_SET_VID_HOST_FD(a);
    	ds_put_format(s, "viro_set_vid_host_fd:%#"PRIx16, ntohs(viro_host->host));
    	break;

    case OFPACT_VIRO_SET_VID_HOST_SRC:
    	viro_host = ofpact_get_VIRO_SET_VID_HOST_SRC(a);
    	ds_put_format(s, "viro_set_vid_host_src:%#"PRIx16, ntohs(viro_host->host));
    	break;

    case OFPACT_VIRO_SET_VID_HOST_DST:
    	viro_host = ofpact_get_VIRO_SET_VID_HOST_DST(a);
    	ds_put_format(s, "viro_set_vid_host_dst:%#"PRIx16, ntohs(viro_host->host));
    	break;
