...
odp_flow_key_attr_len(uint16_t type)
{
    ...
    case OVS_KEY_ATTR_VIRO: return sizeof(struct ovs_key_viro);
                   ...
                   struct ofpbuf *key)
{
...

    {
        ...
        if (sscanf(s, "viro(src_sw=%i,src_host=%i,dst_sw=%i,dst_host=%i,fd_sw=%i,fd_host=%i)%n",
        		&src_sw, &src_host, &dst_sw, &dst_host, &fd_sw, &fd_host, &n) > 0
            && n > 0) {
            struct ovs_key_viro viro_key;

            viro_key.sw_src = htonl(src_sw);
            viro_key.sw_dst = htonl(dst_sw);
            viro_key.sw_fd = htonl(fd_sw);
            viro_key.host_src = htons(src_host);
            viro_key.host_dst = htons(dst_host);
            viro_key.host_fd = htons(fd_host);
            nl_msg_put_unspec(key, OVS_KEY_ATTR_VIRO, &viro_key, sizeof viro_key);
            return n;
        }
    }
