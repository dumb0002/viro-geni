...
pull_viro(struct ofpbuf *packet)
{
	return ofpbuf_try_pull(packet, sizeof(struct viro_hdr));
}
...
static void
parse_viro(struct flow *flow, const struct viro_hdr *vh)
{
	struct viro_addr *va;

	va = (struct viro_addr*) flow->dl_src;

	flow->viro_src_sw = va->sw;
	flow->viro_src_host = va->host;

	va = (struct viro_addr*) flow->dl_dst;

	flow->viro_dst_sw = va->sw;
	flow->viro_dst_host = va->host;

	flow->viro_fd_sw = vh->fd_sw;
	flow->viro_fd_host = vh->fd_host;
}
             ...
             struct flow *flow)
{
    ...
    if (flow->dl_type == htons(ETH_TYPE_VIRO)) {
    	const struct viro_hdr *vh = pull_viro(&b);
    	parse_viro(flow, vh);
    }
