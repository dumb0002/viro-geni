...
static void
parse_named_viro_action(enum ofputil_action_code code, char *arg, struct ofpbuf *ofpacts)
{
	...
	case OFPUTIL_VIRO_AST_PUSH_FD:
		...
		viro_push_fd->fd_sw = htonl(str_to_u32(value));
		ofputil_parse_key_value(&arg, &key, &value);
		viro_push_fd->fd_host = htons(str_to_u16(value, key));
		break;
	...
	case OFPUTIL_VIRO_AST_SET_VID_SW_FD:
		ofpact_put_VIRO_SET_VID_SW_FD(ofpacts)->sw = htonl(str_to_u32(arg));
		break;

	case OFPUTIL_VIRO_AST_SET_VID_SW_SRC:
		ofpact_put_VIRO_SET_VID_SW_SRC(ofpacts)->sw = htonl(str_to_u32(arg));
		break;

	case OFPUTIL_VIRO_AST_SET_VID_SW_DST:
		ofpact_put_VIRO_SET_VID_SW_DST(ofpacts)->sw = htonl(str_to_u32(arg));
		break;

	case OFPUTIL_VIRO_AST_SET_VID_HOST_FD:
		ofpact_put_VIRO_SET_VID_HOST_FD(ofpacts)->host = htons(str_to_u32(arg));
		break;

	case OFPUTIL_VIRO_AST_SET_VID_HOST_SRC:
		ofpact_put_VIRO_SET_VID_HOST_SRC(ofpacts)->host = htons(str_to_u32(arg));
		break;

	case OFPUTIL_VIRO_AST_SET_VID_HOST_DST:
		ofpact_put_VIRO_SET_VID_HOST_DST(ofpacts)->host = htons(str_to_u32(arg));
		break;
                   ...
                   char *arg, struct ofpbuf *ofpacts)
{
    ...
    case OFPUTIL_VIRO_AST_SET_VID_HOST_SRC:
    case OFPUTIL_VIRO_AST_SET_VID_HOST_DST:
    	parse_named_viro_action(code, arg, ofpacts);
    	break;
    }
}
