...
void
packet_set_viro(struct ofpbuf *packet, ovs_be32 dst_sw, ovs_be16 dst_host,
				ovs_be32 src_sw, ovs_be16 src_host,
				ovs_be32 fd_sw, ovs_be16 fd_host)
{
	struct viro_eth_hdr *vh;
...
void
viro_push_fd(struct ofpbuf *packet, ovs_be32 fd_sw, ovs_be16 fd_host)
{
	struct eth_header *eth;
    ...
    memcpy(tmp_vh.eth_dst, eth->eth_dst, ETH_ADDR_LEN);
    memcpy(tmp_vh.eth_src, eth->eth_src, ETH_ADDR_LEN);
    tmp_vh.eth_type = htons(ETH_TYPE_VIRO);
    tmp_vh.fd_sw = fd_sw;
    tmp_vh.fd_host = fd_host;
    tmp_vh.eth_next_type = eth->eth_type;

    vh = ofpbuf_push_uninit(packet, VIRO_HDR_LEN);
...
void
viro_pop_fd(struct ofpbuf *packet)
{
    struct viro_eth_hdr *veh = packet->l2;
    if (packet->size >= sizeof *veh && veh->eth_type == htons(ETH_TYPE_VIRO)) {
        struct eth_header tmp;

        memcpy(tmp.eth_dst, veh->eth_dst, ETH_ADDR_LEN);
        memcpy(tmp.eth_src, veh->eth_src, ETH_ADDR_LEN);
        tmp.eth_type = veh->eth_next_type;

        ofpbuf_pull(packet, VIRO_HDR_LEN);
