...
match_wc_init(struct match *match, const struct flow *flow)
{
    ...
    if (flow->dl_type == htons(ETH_TYPE_VIRO)) {
    	memset(&wc->masks.viro_src_sw, 0xff, sizeof wc->masks.viro_src_sw);
    	memset(&wc->masks.viro_dst_sw, 0xff, sizeof wc->masks.viro_dst_sw);
    	memset(&wc->masks.viro_fd_sw, 0xff, sizeof wc->masks.viro_fd_sw);

    	memset(&wc->masks.viro_src_host, 0xff, sizeof wc->masks.viro_src_host);
    	memset(&wc->masks.viro_dst_host, 0xff, sizeof wc->masks.viro_dst_host);
    	memset(&wc->masks.viro_fd_host, 0xff, sizeof wc->masks.viro_fd_host);
    }
...
static void
format_viro(struct ds *s, const struct flow *f, const struct flow *masks)
{
	/* format viro destination switch (4-bytes) and host (2-bytes) */
	if (masks->viro_dst_sw) {
		ds_put_format(s, "viro_dst_sw=%#08x", ntohl(f->viro_dst_sw));
		if (masks->viro_dst_sw != htonl(UINT32_MAX))
			ds_put_format(s, "/%#08x", ntohl(masks->viro_dst_sw));
		ds_put_cstr(s, ",");
	}
	if (masks->viro_dst_host) {
		ds_put_format(s, "viro_dst_host=%#04x,", ntohs(f->viro_dst_host));
	}

	/* format viro source switch (4-bytes) and host (2-bytes) */
	if (masks->viro_src_sw) {
		ds_put_format(s, "viro_src_sw=%#08x", ntohl(f->viro_src_sw));
		if (masks->viro_src_sw != htonl(UINT32_MAX))
			ds_put_format(s, "/%#08x", ntohl(masks->viro_src_sw));
		ds_put_cstr(s, ",");
	}
	if (masks->viro_src_host) {
		ds_put_format(s, "viro_src_host=%#04x,", ntohs(f->viro_src_host));
	}

	/* format viro forwarding-directive switch (4-bytes) and host (2-bytes) */
	if (masks->viro_fd_sw) {
		ds_put_format(s, "viro_fd_sw=%#08x", ntohl(f->viro_fd_sw));
		if (masks->viro_fd_sw != htonl(UINT32_MAX))
			ds_put_format(s, "/%#08x", ntohl(masks->viro_fd_sw));
		ds_put_cstr(s, ",");
	}
	if (masks->viro_fd_host) {
		ds_put_format(s, "viro_fd_host=%#04x,", ntohs(f->viro_fd_host));
	}
}
...
match_format(const struct match *match, struct ds *s, unsigned int priority)
{
    ...
    if (wc->masks.dl_type) {
        skip_type = true;
        if (f->dl_type == htons(ETH_TYPE_VIRO)) {
        	ds_put_cstr(s, "viro,");
        }
