...
static void
xlate_viro_action(struct action_xlate_ctx *ctx, const struct ofpact *a)
{
    ...
    case OFPACT_VIRO_PUSH_FD:
        ...
        ctx->flow.viro_fd_sw = push_fd->fd_sw;
        ctx->flow.viro_fd_host = push_fd->fd_host;
    	break;

    case OFPACT_VIRO_POP_FD:
    	ctx->flow.viro_fd_sw = htonl(0);
    	ctx->flow.viro_fd_host = htons(0);
    	break;

    case OFPACT_VIRO_SET_VID_SW_FD:
    	ctx->flow.viro_fd_sw = ofpact_get_VIRO_SET_VID_SW_FD(a)->sw;
    	break;

    case OFPACT_VIRO_SET_VID_SW_SRC:
    	ctx->flow.viro_src_sw = ofpact_get_VIRO_SET_VID_SW_SRC(a)->sw;
    	break;

    case OFPACT_VIRO_SET_VID_SW_DST:
    	ctx->flow.viro_dst_sw = ofpact_get_VIRO_SET_VID_SW_DST(a)->sw;
    	break;

    case OFPACT_VIRO_SET_VID_HOST_FD:
    	ctx->flow.viro_fd_host = ofpact_get_VIRO_SET_VID_HOST_FD(a)->host;
    	break;

    case OFPACT_VIRO_SET_VID_HOST_SRC:
    	ctx->flow.viro_src_host = ofpact_get_VIRO_SET_VID_HOST_SRC(a)->host;
    	break;

    case OFPACT_VIRO_SET_VID_HOST_DST:
    	ctx->flow.viro_dst_host = ofpact_get_VIRO_SET_VID_HOST_DST(a)->host;
    	break;
        ...
        if (trace.ctx.slow) {
            ...
            for (slow = trace.ctx.slow; slow; ) {
                ...
                case SLOW_MATCH:
                    ...
                    break;
                case SLOW_VIRO:
                	ds_put_cstr(ds, "\n\t- Needs viro slow-path processing.");
                	break;
                }
