...
class rdv_withdraw(packet_base):
  MIN_LEN = 4
  ...
  def _to_str(self):
    s = "[RDVWithdraw failed_node={0}".format(str(self.failed_node))
  ...
  def parse(self, raw):
    ...
    hdr = struct.unpack("!I", raw[:rdv_withdraw.MIN_LEN])

    self.failed_node = VidAddr(hdr[0], 0)

    self.parsed = True    
    

  def hdr(self, payload):
    buf = struct.pack("!I", self.failed_node.sw)
    return buf
